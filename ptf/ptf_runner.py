...
# Maximilian Pudelko (max@opennetworking.com)
#
...
class StreamChannelWrapper():
    def __init__(self, stub):
        self.stub = stub
        self.q_in = Queue.Queue()
        self.q_out = Queue.Queue()
        self.iter = self._out_iter(self.q_out)

    def _out_iter(self, q):
        while True:
            p = self.q_out.get()
            if p is None:
                break
    ...
    def put(self, request):
        return self.q_out.put(request)

    def get(self, alt=None):
        try:
    ...
    def __enter__(self):
        self.stream = self.stub.StreamChannel(self.iter)
        return self
...
def update_config(config_path, p4info_path, grpc_addr, device_id):
    ...
    info("Sending master arbitration")
    ...
    with StreamChannelWrapper(stub) as stream:    
        ...
        with open(p4info_path, 'r') as p4info_f:
            google.protobuf.text_format.Merge(p4info_f.read(), request.config.p4info)
        with open(config_path, 'rb') as config_f:
            request.config.p4_device_config = config_f.read()
