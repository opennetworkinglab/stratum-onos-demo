...
class P4RuntimeTest(BaseTest):
    def setUp(self):
        BaseTest.setUp(self)
        self.device_id = 1
        self.election_id_low = 1
        self.election_id_high = 0
    ...
    def set_action(self, action, a_name, params):
        ...
        if action_id is None:
            self.fail("Failed to get id of action %s - perhaps the action name is misspelled?" % a_name)
    ...
    def _get_new_write_request(self):
    ...
    def undo_write_requests(self, reqs):
        ...
        new_req = self._get_new_write_request()
